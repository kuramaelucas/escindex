...
> **Pendência aberta (23/09/2026):** rodar de novo o `nuvem/esquema.sql` no painel do Supabase. Quatro novidades dependem dele — a anotação pessoal da questão salva (`favoritos.nota`), a contagem de flashcards por dia (`dias_cartoes.quantidade`), os flashcards favoritados (tabela `favoritos_cartoes`) e as **questões escondidas** (tabela `questoes_ocultas`) —, e o arquivo já traz as linhas que acrescentam cada uma sem mexer no que existe. Enquanto não for rodado, a plataforma funciona e não perde nada: ela pula o que falta, sincroniza todo o resto e avisa em *Perfil* o que está faltando (seção 4-B).
